    ...
    (() => {
        ...
        class MousePRLX {
            constructor(props, data = null) {
                ...
                } else this.setLogging("Инициализация отключена");
            }
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            if (formNameInput) formNameInput.addEventListener("input", (function(event) {
        ...
        window.addEventListener("scroll", (function() {
            if (window.location.pathname === "/home.html") {
                ...
                const mainBodyElement = document.querySelector(".main__body");
                const buttonElement = document.querySelector(".main__button-scroll");
                const startScroll = 200;
                const scaleValue = 1 - Math.max(scrollY - startScroll, 0) * .001;
                const limitedScale = Math.max(scaleValue, .8);
                mainBodyElement.style.transform = `translateY(-${scrollY * .3}px) scale(${limitedScale})`;
                buttonElement.style.transform = `translateY(-${scrollY * .5}px) scale(${limitedScale})`;
            }
        }));
        document.addEventListener("DOMContentLoaded", (function() {
            const header = document.getElementById("header");
            const headerHeight = header.clientHeight;
            const headerActiveSection = document.querySelector("[data-header-active]");
            if (!headerActiveSection) return;
            const headerActiveOffsetTop = headerActiveSection.offsetTop;
            let isHeaderHidden = false;
            function updateHeader() {
                if (window.scrollY >= headerActiveOffsetTop - headerHeight) {
                    if (!header.classList.contains("header-scrolled")) header.classList.add("header-scrolled");
                    if (isHeaderHidden) {
                        isHeaderHidden = false;
                        requestAnimationFrame((function() {
                            header.classList.remove("header-scrolled-hidden");
                        }));
                    }
                } else if (!isHeaderHidden && header.classList.contains("header-scrolled")) {
                    header.classList.add("header-scrolled-hidden");
                    isHeaderHidden = true;
                    setTimeout((function() {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            const tabs = document.querySelectorAll(".content-tabs-portfolio");
